...
class UsersController extends AbstractController
{
    ...
    public function showPersonneByEmail(string $email): JsonResponse
    {
        $userToShow = $this->userRepository->findOneBy(['email' => $email]);
        return $this->json($userToShow, 200, [], [
            'skip_null_values'=>true,
            'ignored_attributes'=> ['personnes','roles','__isInitialized__','consultationsOfUser']
        ]);
    ...
    public function addPersonne(Request $request ):JsonResponse
    {
        $person = new User();
        $role = new Role();
        if (!is_null($request->request->get('dateDip'))){
            $role = $this->roleRepository->findOneBy(['nomRole'=>'medecin']);
            $person->setNom($request->request->get('nom'));
            $person->setPrenom($request->request->get('prenom'));
            $person->setDateNaiss($request->request->get('dateNaiss'));
            $person->setAdresse($request->request->get('adresse'));
            $person->setEmail($request->request->get('email'));
            $person->setTelephone($request->request->get('telephone'));
            $person->setPays($request->request->get('pays'));
            $person->setMaladie($request->request->get('maladie'));
            $person->setPassword($this->encoder->encodePassword($person, $request->request->get('password')));
            $person->setDateDip($request->request->get('dateDip'));
            $person->setAdresseCab($request->request->get('adresseCab'));
        } elseif(!is_null($request->request->get('maladie'))){
            $role = $this->roleRepository->findOneBy(['nomRole'=>'user']);
            ...
            $role = $this->roleRepository->findOneBy(['nomRole'=>'admin']);
            $person->setPseudo($request->request->get('pseudo'));
            $person->setEmail($request->request->get('email'));
            $person->setPassword($this->encoder->encodePassword($person, $request->request->get('password')));
        }
